      ...
      <nav className="w-full bg-blue-600 text-white py-4">
        <div className="container mx-auto flex justify-between items-center px-6">
          <div className="space-x-4 text-lg font-bold">
            {/* <Link to="/user/createabooking" className="hover:underline">Create Booking</Link> */}
            <Link to="/user/userbookings" className="hover:underline">Create Bookings</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {!loading && bookings.length === 0 ? (
          ...
          bookings.map((booking) => (
            <div key={booking['booking Id']} className="bg-black shadow-md rounded-lg p-4">
              <h3 className="text-xl font-bold mb-2">{booking['court_name']}</h3>
              <p><strong>Booking ID:</strong> {booking['_id']}</p>
              <p><strong>City:</strong> {booking.centre}</p>
